...
class SimpleController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Simple Velocity Controller has been started.")
        ...
        self.get_logger().info(
            "Speed conversion Maxtrix: %s" % str(self.speed_conversion_)
        )
        ...
        self.transfrom_stamped_.child_frame_id = "base_footprint"
    ...
    def jointCallback(self, msg):
        # self.get_logger().info(
        #     f"JointState names: {msg.name if hasattr(msg, 'name') else 'N/A'}"
        # )
        self.get_logger().info(f"JointState positions: {msg.position}")
        ...
        angular = (
            self.wheel_radius_ * fi_right - self.wheel_radius_ * fi_left
        ) / self.wheel_separation_

        d_s = (self.wheel_radius_ * dp_right + self.wheel_radius_ * dp_left) / 2

        d_theta = (
            self.wheel_radius_ * dp_right - self.wheel_radius_ * dp_left
        ) / self.wheel_separation_
        ...
        self.transfrom_stamped_.transform.rotation.x = q[0]
        self.transfrom_stamped_.transform.rotation.y = q[1]
        self.transfrom_stamped_.transform.rotation.z = q[2]
        self.transfrom_stamped_.transform.rotation.w = q[3]
        self.transfrom_stamped_.header.stamp = self.get_clock().now().to_msg()

        self.br_.sendTransform(self.transfrom_stamped_)
        self.get_logger().info(
            "current x: %.3f, y: %.3f, theta: %.3f" % (self.x_, self.y_, self.theta_)
        )
